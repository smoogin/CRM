...
import { saveMarkupCalc, deleteMarkupCalc } from "@/lib/actions/markup";
...
export function MarkupCalculator({ saved }: { saved: SavedCalc[] }) {
  ...
  function load(c: SavedCalc) {
    setFields({
      quantity: fmt(c.quantity, "qty"),
      unitCost: fmt(c.unitCost, "unit"),
      unitSell: fmt(c.unitSell, "unit"),
      markup: fmt(c.markup, "pct"),
      totalCost: fmt(c.totalCost, "money"),
      totalSell: fmt(c.totalSell, "money"),
    });
        ...
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          <div className="card grid grid-cols-2 divide-x divide-slate-100 p-0">
            <div className="p-5">
              ...
              >
                {Number.isFinite(profit) ? `$${fmt(profit, "money")}` : "—"}
              </div>
            </div>
            <div className="p-5">
              <div className="text-xs font-medium uppercase tracking-wide text-slate-400">
          ...
          <div className="card flex flex-col gap-3 p-5 sm:flex-row sm:items-end">
            <div className="flex-1">
              <label className="label" htmlFor="calcLabel">
                Save this calculation
              </label>
              <input
                id="calcLabel"
                className="input"
                placeholder="Name (optional) e.g. Acme cereal carton"
                value={label}
          ...
          <div className="card">
            ...
            {saved.length === 0 ? (
              <p className="px-5 py-6 text-sm text-slate-400">
                No saved calculations yet. Fill in the calculator and click “Save
                calculation”.
              </p>
            ) : (
              <div className="divide-y divide-slate-100">
                {saved.map((c) => {
                  const profit = c.totalSell - c.totalCost;
                  return (
                    <div
                      key={c.id}
                    ...
                    >
                      ...
                      >
                        <div className="truncate text-sm font-medium text-slate-800 hover:text-brand-600">
                          {c.label ?? "Untitled calculation"}
                        </div>
                        <div className="mt-0.5 text-xs text-slate-500">
                          {c.quantity.toLocaleString()} × ${fmt(c.unitCost, "unit")}{" "}
                          → ${fmt(c.unitSell, "unit")} · {fmt(c.markup, "pct")}%
                          markup
                        </div>
                        <div className="mt-0.5 text-xs text-slate-500">
                          Sell ${fmt(c.totalSell, "money")} ·{" "}
                          <span
                            className={
                              profit < 0 ? "text-red-600" : "text-emerald-600"
                            }
                          >
                            Profit ${fmt(profit, "money")}
                          </span>{" "}
